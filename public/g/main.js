const setName = async () => {
    try {
        ...
        const result = await fetch('http://localhost:5000/g/id2Name/'+ id)
        if (result.ok) {
...
const getStories = async () => {
    try {
        const result = await fetch('http://localhost:5000/s/pending');
        if (result.ok) {
            const data = await result.json();
            data.forEach(story => {
                const div = document.createElement('div');
...
const toggleSort = async (e) => {
    ...
    if (e.currentTarget.querySelectorAll('p')[0].textContent === 'Date'){
        ...
        url = 'http://localhost:5000/s/sortedByDate?type=' + date_sort;
    } else {   // AMOUNT
        amount_sort = (amount_sort === 'asc' ? 'desc' : 'asc');    
        url = 'http://localhost:5000/s/sortedByAmount?type=' + amount_sort;
    }
